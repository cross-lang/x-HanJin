...
`汉津（HanJin）`是一个基于 Gin 框架深度封装的生产级 Go Web 项目框架，采用标准 Go 项目布局（cmd / pkg / internal），预集成数据库、消息队列、缓存、日志、中间件、加密工具等常用组件，适用于中小型 Web 后端、微服务模块及需要快速迭代与多数据源集成的业务系统。

## 核心价值
- **开箱即用** — 预置生产级基础组件，快速启动业务开发
- **标准规范** — 遵循 Go 项目标准布局和最佳实践
- **高度模块化** — 清晰的分层架构（Controller → Service → Database），易于扩展
- **企业级特性** — 完善的日志、加密、签名验证与容器化部署支持
